...
const existingWebhook = webhookEndpoints.data.find( ({ url }) => url === WEBHOOK_URL );
if (existingWebhook){
...
app.get('/settings', async (req, res) => {
    ...
    if (account.settings.branding.logo) {
        const iconFile = await stripe.files.retrieve(account.settings.branding.icon);
...
app.post('/cart/line-item', async (req, res) => {
    ...
    res.send(await commerceTools.cartAddLineItem(cartId, productId,variantId, version));
});
...
app.post('/create-checkout-session', async (req, res) => {
    ...
    try {
        ...
        if (currency === 'eur') {
            ...
            payload.shipping_address_collection.allowed_countries = ['FR', 'NL', 'GB', 'DE']
        }
        if (currency === 'gbp') {
...
app.post("/create-payment-intent", async (req, res) => {
    ...
    const payment = await commerceTools.createPayment(paymentIntent, currency, total)
...
app.post("/events", async (req, res) => {
    if (req.body.type == "payment_intent.succeeded"){
        ...
        commerceTools.updateOrder(paymentIntent.metadata.commerceToolsOrderId, "Paid", paymentIntent)
    ...
    if (req.body.type == "payment_intent.payment_failed"){
        ...
        commerceTools.updateOrder(paymentIntent.metadata.commerceToolsOrderId, "Failed", paymentIntent) 
