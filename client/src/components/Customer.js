            ...
            <div style={styles.panel}>
                <div className="row mb-2">
                    <div className="col">
                        ...
                        <input type="text" className="form-control" id="email" onChange={e => setEmail(e.target.value)} onBlur={checkCustomerExists} value={email} />
                    </div>
                    <div className="col">
                        <label htmlFor="name">Password (N/A)</label>
                        <input type="password" className="form-control" id="password" value="aaaaa" onChange={doNothing} />
                    </div>
                </div>
                <div className="row mb-2">
                    <div className="col">
                ...
                <div className="row mb-2">
                    <div className="col">
                        {email !== '' && custExists && <button className="btn btn-secondary btn-sm" style={{ marginRight: 5 }} disabled={true} onClick={createCustomer}>
                            {loading ? <div className="spinner-border spinner-border-sm" role="status"></div> : "Update Account"}
                        </button>}
                        {email !== '' && !custExists && <button className="btn btn-secondary btn-sm" style={{ marginRight: 5 }} onClick={createCustomer}>
                            {loading ? <div className="spinner-border spinner-border-sm" role="status"></div> : "Create Account"}
                        </button>}
                        {(email !== '' || name !== '' || address !== '' || city !== '' || country !== '' ) &&
                            <button className="btn btn-secondary btn-sm" onClick={resetCustomer}>Reset Form</button>
                        }
                    </div>
                </div>
            </div>
