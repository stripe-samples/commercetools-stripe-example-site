...
export default function ProductCard(props) {
            ...
            output.push(
                Object.entries(metadata).map(([key, value]) => {
                    if (key.indexOf(type) !== -1) {
                        return type === 'Features' ? <li key={key}>{value}</li> : <p key={key}>{value}</p>
                    }
                    return ''
                })
            )
    ...
    const displayPrice = (value) => {
        ...
        return `${getSymbolFromCurrency(props.currency)} ${(value.centAmount / 100).toFixed(value.fractionDigits)}`
        ...
        <>
            ...
            <Modal show={show} centered onHide={handleClose} size="lg">
                <Modal.Header style={styles.nameModal}>
                    {props.product.masterData.current.name['en-US']}
                    <FontAwesomeIcon icon={faTimes} style={{cursor: 'pointer'}} onClick={handleClose} />
                </Modal.Header>
                <Modal.Body>
                    <div className="row">
                        <div className="col-5">
                ...
                <Modal.Footer>
                    <button className="btn" style={{backgroundColor: props.brandColor, color: 'blue'}} id={props.product.id} onClick={addToCart}>Add to Cart</button>
                </Modal.Footer>
            </Modal>
        </>
    );
