...
export default function Checkout(props) {
    ...
    const handleChange = (e) => {
        ...
        setShowElements(e.target.value === 'elements');
        setShowUPE(e.target.value === 'upe');
    }

    return (
        <>
            {props.cart && <div className="row" >
                <div className="col-6">
                    <Cart cart={props.cart} currency={props.currency} />
                </div>
                <div className="col-6">
                    <Customer setCustId={setCustId} setCustInfo={setCustInfo} setCart={props.setCart} cart={props.cart} custId={custId} />
                    {custId && <>
                        <h4 style={styles.header}>Checkout</h4>
                        {/* If you want to only show one way to checkout, remove or comment out this <select> element  */}
                        <select className="form-control" style={styles.selector} disabled={custId === ''} defaultValue='' onChange={handleChange}>
                    ...
                    {custId && showHosted && <CheckoutHosted cart={props.cart} custId={custId} brandColor={props.brandColor} currency={props.currency}/>}
                    {custId && showUPE && <CheckoutUPE cart={props.cart} custId={custId} custInfo={custInfo} brandColor={props.brandColor} currency={props.currency}/>}
                </div>
            </div>
            }
            {!props.cart &&
                <div className="row">
                    <div className="col">Your cart is empty
                    </div>
                </div>
            }
        </>
